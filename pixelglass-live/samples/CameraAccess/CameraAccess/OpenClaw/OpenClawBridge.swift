...
class OpenClawBridge: ObservableObject {
  ...
  /// Converts the configured host (which may have ws/wss/http/https scheme) into an https:// or http:// base URL with port if local.
  private static func httpBaseURL() -> String {
    let raw = GeminiConfig.openClawHost
    let useSecure = raw.hasPrefix("wss://") || raw.hasPrefix("https://")
    let stripped = raw
    ...
    let isLocal = stripped.hasSuffix(".local") || stripped == "localhost" || stripped.starts(with: "127.") || stripped.starts(with: "192.168.")
    if isLocal {
      return "\(scheme)://\(stripped):\(GeminiConfig.openClawPort)"
    }
  ...
  func checkConnection() async {
    ...
    guard let url = URL(string: "\(OpenClawBridge.httpBaseURL())/v1/chat/completions") else {
